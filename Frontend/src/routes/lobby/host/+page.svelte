<script>
  ...
  onMount(async () => {
    ...
    try {
      ...
      if (!res.ok) { error = i18n.t.roomCreateError; loading = false; return; }
      const data = await res.json();
      code = data.code;
      quizTitle = data.quizTitle;
      game.currentGameCode = code;
    } catch {
      error = i18n.t.backendError;
      loading = false;
      return;
    }
...
<div class="container" style="max-width:500px; padding-top:2rem; padding-bottom:3rem;">
  ...
  {:else}
    <div class="text-center mt-4 mb-4">
      ...
      <p class="text-muted mt-2">{i18n.t.shareCodeHint}</p>
      <div style="font-size:2.8rem; font-weight:800; letter-spacing:8px;
        border:3px solid #1a1a1a; border-radius:8px; padding:12px 24px;
        display:inline-block; background:#fffef5; box-shadow:4px 4px 0 #1a1a1a;">
        {code}
      </div>
    </div>
    ...
    <div class="card-sketch p-3 mb-4">
      <div class="d-flex justify-content-between align-items-center mb-2">
        ...
        <span class="text-muted small">
          <span style="display:inline-block; width:8px; height:8px; border-radius:50%;
            background:#2a9d5c; margin-right:4px;"></span>
          {i18n.t.waitingForPlayers}
        </span>
      </div>
      ...
      {/each}
      {#if players.length === 0}
        <div class="text-muted text-center py-2 small">{i18n.t.waitingForPlayers}</div>
      {/if}
    </div>

    <button class="btn btn-sketch btn-sketch-black w-100 py-2 mb-2"
      onclick={handleStart} disabled={starting || players.length < 1}>
      {starting ? i18n.t.starting : i18n.t.startGame}
    </button>
    <div class="text-center text-muted small">{i18n.t.hostOnlyHint}</div>
  {/if}
</div>