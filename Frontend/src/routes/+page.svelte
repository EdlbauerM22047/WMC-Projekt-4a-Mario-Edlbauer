<script>
  ...
  function handleJoin() {
    nicknameError = ""; codeError = "";
    if (!game.nickname.trim()) { nicknameError = i18n.t.noNickname; return; }
    if (!gameCode.trim()) { codeError = i18n.t.noCode; return; }
...
<div class="container" style="max-width:420px; padding-top:2.5rem;">
  ...
  <div class="card-sketch p-4">
    <div class="mb-3">
      ...
      <input class="form-control input-sketch" placeholder={i18n.t.nicknamePlaceholder}
        bind:value={game.nickname} maxlength="20" />
      {#if nicknameError}<div class="text-danger small mt-1">{nicknameError}</div>{/if}
    </div>
    ...
    <div class="mb-3">
      ...
      <input class="form-control input-sketch text-center"
        style="font-size:1.4rem; letter-spacing:6px; font-weight:700;"
        placeholder={i18n.t.gameCodePlaceholder}
        bind:value={gameCode} maxlength="5"
        oninput={() => (gameCode = gameCode.toUpperCase())} />
      {#if codeError}<div class="text-danger small mt-1">{codeError}</div>{/if}
    </div>
