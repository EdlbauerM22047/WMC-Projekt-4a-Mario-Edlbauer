<script>
  ...
  async function handleCreate() {
    formError = "";
    if (!newTitle.trim()) { formError = "Titel ist erforderlich!"; return; }
    try {
      creating = true;
...
<div class="container" style="max-width:600px; padding-top:2rem; padding-bottom:3rem;">
  ...
  <div class="d-flex align-items-center justify-content-between mt-3 mb-3">
    ...
    <button class="btn btn-sketch btn-sketch-black px-3 py-2"
  ...
  {#if showCreateForm}
    <div class="card-sketch p-4 mb-4">
      ...
      <div class="mb-3">
        ...
        <input class="form-control input-sketch" bind:value={newTitle} maxlength="80" />
      </div>
      <div class="mb-3">
        <label class="form-label fw-bold">{i18n.t.quizDescription}</label>
        <textarea class="form-control input-sketch" rows="2" bind:value={newDescription} maxlength="200"></textarea>
      </div>
      {#if formError}<div class="text-danger small mb-2">{formError}</div>{/if}
      <button class="btn btn-sketch btn-sketch-black w-100 py-2" onclick={handleCreate} disabled={creating}>
  ...
  {:else if quizzes.length === 0}
    <div class="card-sketch p-4 text-center text-muted">{i18n.t.noQuizzes}</div>
  {:else}
      ...
      <div class="card-sketch p-3 mb-3">
        <div class="d-flex align-items-start gap-3">
          ...
          <div class="flex-grow-1">
            ...
            {#if quiz.description}<div class="text-muted small">{quiz.description}</div>{/if}
            <div class="text-muted small mt-1">{quiz.question_count} {i18n.t.questions}</div>
          </div>
          <div class="d-flex flex-column gap-2">
            <button class="btn btn-sketch btn-sketch-black btn-sm px-2 py-1" style="font-size:0.8rem;"
              onclick={() => { game.currentQuizId = quiz.id; goto(`/lobby/host?quizId=${quiz.id}`); }}>
              {i18n.t.hostGame}
            </button>
            <button class="btn btn-sketch btn-sketch-outline btn-sm px-2 py-1"
